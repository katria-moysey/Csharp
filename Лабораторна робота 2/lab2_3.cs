...
namespace Program
{
    class Rhombus
    {
        static void Main()
        {
            ...
            double AB= Math.Pow((x_2 - x_1), 2) + Math.Pow((y_2 - y_1), 2);
            double BC = Math.Pow((x_3 - x_2), 2) + Math.Pow((y_3 - y_2), 2);
            double CD= Math.Pow((x_4 - x_3), 2) + Math.Pow((y_4 - y_3), 2);
            double DA = Math.Pow((x_1 - x_4), 2) + Math.Pow((y_1 - y_4), 2);
            Console.WriteLine("Введіть допустиму похибку e:");
            double e =double.Parse(Console.ReadLine());
            if (Math.Abs(AB-BC) <e && Math.Abs(CD-DA) <e && Math.Abs(CD-AB) <e )
            {
